...
namespace Lab8
{
    class Program
    {
        ...
        public static void LearnMore(int index)
        {
            try
            {
                Console.WriteLine($"\nWhat would you like to know about {Names[index - 1]}? ((H)ome town, (F)avorite food, or (E)verything");
                string userAnswer = Console.ReadLine().ToLower();
                int listIndex = index - 1;
                if (userAnswer == "home town" || userAnswer == "h")
                {
                    Console.WriteLine($"{Names[listIndex]} grew up in {HomeTowns[listIndex]}.");
                    Console.ReadKey();
                }
                else if (userAnswer == "favorite food" || userAnswer == "f")
                {
                    Console.WriteLine($"{Names[listIndex]}'s favorite food is {FavoriteFoods[listIndex]}");
                    Console.ReadKey();                }
